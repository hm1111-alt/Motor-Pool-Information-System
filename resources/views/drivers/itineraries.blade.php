<x-app-layout>
            ...
            @else
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                                ...
                                <div class="relative rounded-md shadow-sm">
                                    ...
                                    @if($search)
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                                            <a href="{{ route('driver.itineraries') }}" class="text-gray-400 hover:text-gray-600">
                                    ...
                                    @forelse($itineraries as $itinerary)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ $itinerary->purpose ?? 'N/A' }}</div>
                                                @if($itinerary->travelOrder)
                                                    <div class="text-sm text-gray-500">TO: {{ $itinerary->travelOrder->purpose ?? 'N/A' }}</div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                @if($itinerary->vehicle)
                                                    ...
                                                    <div class="text-xs text-gray-400">Plate: {{ $itinerary->vehicle->plate_number }}</div>
                                                @else
                        ...
                        <!-- Pagination -->
                        <div class="mt-6">
                            {{ $itineraries->appends(['search' => $search, 'status' => $status])->links() }}
                        </div>
                    </div>
                </div>
            @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateUrl() {
            ...
            let url = new URL('{{ route("driver.itineraries") }}', window.location.origin);
