            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="border-b border-gray-200">
                    <nav class="flex -mb-px">
                        <a href="{{ route('itinerary.approvals.vp.pending', ['tab' => 'pending']) }}" 
                        ...
                        <a href="{{ route('itinerary.approvals.vp.pending', ['tab' => 'approved']) }}" 
                        ...
                        <a href="{{ route('itinerary.approvals.vp.pending', ['tab' => 'cancelled']) }}" 
                ...
                <div class="p-6 text-gray-900">
                                ...
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        @if(($tab ?? 'pending') === 'pending')
                                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Unit Head Approval</th>
                                        ...
                                        <tr>
                                            ...
                                            @if(($tab ?? 'pending') === 'pending')
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    ...
                                                    <form method="POST" action="{{ route('itinerary.approvals.vp.approve', $itinerary->id) }}" class="inline">
                                                        @csrf
                                                        @method('PUT')
                                                    ...
                                                    <form method="POST" action="{{ route('itinerary.approvals.vp.reject', $itinerary->id) }}" class="inline">
                                                        @csrf
                                                        @method('PUT')
                                            ...
                                            @else
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    @if($itinerary->vp_approved_by)
                                                        {{ App\Models\User::find($itinerary->vp_approved_by)?->name ?? 'Unknown User' }}
                                                    @else
                                                        N/A
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    @if($itinerary->vp_approved_at)
                                                        {{ $itinerary->vp_approved_at->format('M d, Y g:i A') }}
                                                    @else
                                                        N/A
                                                    @endif
                                                </td>
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            @if(($tab ?? 'pending') === 'pending')
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No itineraries pending for VP approval</h3>
                            ...
                            @elseif(($tab ?? 'pending') === 'approved')
                                ...
                                <p class="mt-1 text-sm text-gray-500">There are currently no itineraries approved by the VP.</p>
                            @else
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No cancelled itineraries</h3>
                                <p class="mt-1 text-sm text-gray-500">There are currently no itineraries rejected by the VP.</p>
                            @endif
                        </div>
                    @endif
                </div>
