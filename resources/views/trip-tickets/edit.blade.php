...
@section('content')
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('trip-tickets.update', $tripTicket) }}">
                        @csrf
                        @method('PUT')
                                        ...
                                        <select name="itinerary_id" id="itinerary_id" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-[#1e6031] focus:ring focus:ring-[#1e6031] focus:ring-opacity-50" required onchange="updateItineraryDetails()">
                                            ...
                                            @foreach($itineraries as $itinerary)
                                                <option value="{{ $itinerary->id }}" 
                                                    ...
                                                    {{ $tripTicket->itinerary_id == $itinerary->id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2 bg-blue-50 p-4 rounded-lg">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div id="driver-preview" class="w-full rounded-lg border-gray-300 bg-gray-100 p-2 min-h-[40px]">{{ $tripTicket->itinerary->driver?->first_name . ' ' . $tripTicket->itinerary->driver?->last_name ?? 'Select an itinerary to see details' }}</div>
                                    </div>
                                    
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Vehicle</label>
                                        <div id="vehicle-preview" class="w-full rounded-lg border-gray-300 bg-gray-100 p-2 min-h-[40px]">{{ $tripTicket->itinerary->vehicle?->model ?? 'Select an itinerary to see details' }}</div>
                                    </div>
                                    
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Destination</label>
                                        <div id="destination-preview" class="w-full rounded-lg border-gray-300 bg-gray-100 p-2 min-h-[40px]">{{ $tripTicket->itinerary->destination ?? 'Select an itinerary to see details' }}</div>
                                    </div>
                                    
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Purpose</label>
                                        <div id="purpose-preview" class="w-full rounded-lg border-gray-300 bg-gray-100 p-2 min-h-[40px]">{{ $tripTicket->itinerary->purpose ?? 'Select an itinerary to see details' }}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="md:col-span-2 bg-gray-50 p-4 rounded-lg">
                                ...
                                <div id="passenger-list" class="mt-4">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <span id="head-of-party-info" class="text-sm text-gray-500">Head of Party: <span id="current-head">{{ $tripTicket->head_of_party ?? 'None selected' }}</span></span>
                                    </div>
                                    <ul id="passenger-items" class="list-disc list-inside bg-white p-2 rounded border">
                                        @if($tripTicket->passengers && is_array($tripTicket->passengers))
                                            @foreach($tripTicket->passengers as $passenger)
                                                <li class="passenger-item flex justify-between items-center" @if($tripTicket->head_of_party == $passenger) style="background-color: #fef3c7; border-left: 4px solid #f59e0b;" @endif>
                                                    <div>{{ $passenger }}</div>
                                                    <div>
                                                        <button type="button" class="text-sm bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 rounded ml-2" onclick="setAsHeadOfParty('{{ $passenger }}', '{{ $passenger }}', this)">Set as Head</button>
                                                        <button type="button" class="text-red-600 ml-2 delete-passenger-btn" onclick="removePassenger(this)">×</button>
                                                    </div>
                                                </li>
                                            @endforeach
                                        @endif
                                    ...
                                    <input type="hidden" name="head_of_party" id="head_of_party" value="{{ $tripTicket->head_of_party ?? '' }}">
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function removePassenger(button) {
            ...
            const passengerName = listItem.querySelector('div').textContent.split('(')[0].trim();
        ...
        // Initialize the details when the page loads
        document.addEventListener('DOMContentLoaded', function() {
            updateItineraryDetails();
            
            // Add hidden inputs for existing passengers
            const existingItems = document.querySelectorAll('#passenger-items .passenger-item');
            for (let item of existingItems) {
                const passengerName = item.textContent.split('(')[0].trim();
            ...
            // Initial refresh of travel order dropdown based on existing passengers
            refreshTravelOrderDropdown();
            
            // Initialize head of party highlighting
            const initialHeadOfParty = document.getElementById('head_of_party').value;
            if (initialHeadOfParty && initialHeadOfParty !== '') {
                // Find the list item containing the head of party name and highlight it
                const allPassengerItems = document.querySelectorAll('#passenger-items li');
                allPassengerItems.forEach(item => {
                    if (item.textContent.includes(initialHeadOfParty)) {
                        item.style.backgroundColor = '#fef3c7'; // yellow-100
                        item.style.borderLeft = '4px solid #f59e0b'; // yellow-500
                    }
                });
            }
        });
