...
<tr class="division-row hover:bg-gray-50 transition duration-150" 
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $division->division_abbr }}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $division->office->office_name ?? 'N/A' }}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $division->division_code }}</td>
    <td class="px-6 py-4 whitespace-nowrap">
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
        <a href="{{ route('admin.divisions.edit', $division) }}" class="text-[#1e6031] hover:text-[#164f2a] mr-3 inline-flex items-center">
            <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            Edit
        </a>
        <form action="{{ route('admin.divisions.destroy', $division) }}" method="POST" class="inline delete-form">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-red-600 hover:text-red-900 inline-flex items-center delete-btn" data-name="{{ $division->division_name }}">
                <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
