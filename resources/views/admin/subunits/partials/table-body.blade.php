...
<tr class="subunit-row hover:bg-gray-50 transition duration-150" 
    data-name="{{ strtolower($subunit->subunit_name) }}" 
    data-abbr="{{ strtolower($subunit->subunit_abbr) }}" 
    data-unit="{{ strtolower($subunit->unit->unit_name ?? '') }}" 
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subunit->unit->unit_name ?? 'N/A' }}</td>
    <td class="px-6 py-4 whitespace-nowrap">
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
        <a href="{{ route('admin.subunits.edit', $subunit) }}" class="text-[#1e6031] hover:text-[#164f2a] mr-3 inline-flex items-center edit-subunit">
        ...
        <form action="{{ route('admin.subunits.destroy', $subunit) }}" method="POST" class="inline delete-form">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-red-600 hover:text-red-900 inline-flex items-center delete-subunit" data-name="{{ $subunit->subunit_name }}">
                <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<tr id="no-results-row">
    <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
        <div class="flex flex-col items-center justify-center py-8">
            ...
            <p class="text-lg font-medium text-gray-900">No subunits found</p>
